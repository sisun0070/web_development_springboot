...
import me.hwangjisun.springbootdeveloper.domain.Article;
import me.hwangjisun.springbootdeveloper.dto.AddArticleRequest;
import me.hwangjisun.springbootdeveloper.dto.ArticleResponse;
import me.hwangjisun.springbootdeveloper.dto.UpdateArticleRequest;
import me.hwangjisun.springbootdeveloper.service.BlogService;
...
public class BlogApiController {
    ...
    /*
        ...
        4. ResponseEntity.status().body()는 응답 코드로 201, 즉 Created !
            테이블에 저장된 객체를 반환합니다.
        ...
        API가 잘 동작하는지 테스트 해볼 예정
            ...
            -> 여기까지 성공했다면 스프링 부트 서버에 저장도니 것을 의미합니다.

            여기까지가 HTTP 메서드 POST 서버에 요청을 한 후에 값을 저장하는 과정에 해당.

            이제 크롬 켜세요
            http://localhost:8080/h2-console -> 주소창에 입력
            ...
            내일은 반복작업을 줄여줄 테이스 코드들을 작성하겠습니다.
                - 매번 H2 들어가는게 번거로워서
                test를 이용할 예정.
     */
    ...
    /*
        @PathVariable : URL에서 값을 가져오는 애너테이션.
            ...
            그리고 이 값은 바로 전에 만든 서비스 클래스와 findById() 메서드로 넘어가서 3번 블로그
    ...
    /*
        ...
        GET http://localhost8080/api/article
        ...
        조회 성공하셨으면
        DELETE HTTP 메서드를 바꿔주고,
        http://localhost8080/api/article/1 하고 Send 버튼 누릅니다.

        GET으로 HTTP 메서드 바꿔주고,
        http://localhost8080/api/article/1 -> 지워졌기 때문에 조회 X

        http://localhost8080/api/article -> 얘로 조회해서 전체 글 목록이 줄었는지 확인
