    ...
    사전 지식 : OAuth
        ...
        관련 용어 정리
            ...
            리소스 서버(Resource Server) : 리소스 오너의 정보를 가지며, 리소스 오너의 정보를
                보호하는 주체를 의미하고, 네이버, 구글, 카카오, 페이스북에 리로스 서버에 해당.
        ...
        OAuth를 사용하면 인증 서버에서 발급받은 토큰을 사용해서 리소스서버에
            ...
            리소스 오너 정보를 취득하는 네 가지 방법
                1) 권한 부여 코드 승인 타입(Authorization Code Crant Type) :
                    OAuth 2.0에서 가장 대중적인 인증 방법. 클라이언트가 리소스에 접근한는 데 사용.
                    권한에 접근할 수 잇는 코드와 리소스 어노에 대한 액세스 토큰을 발급받는 방식.
                ...
                3) 리소스 소유자 암호 자격증명 승인 타입(Resource Owner Password Credentials) :
                    클라이언트의 패스워드르 이용해서 액세스 토큰에 대한 사용자의 자격 증명을 교환하는 방식

                4) 클라이언트 자격 증명 승인 타입(Client Credentials Grant) :
                    클라이언트 컨텍스트 외부에서 액세스 토큰을 얻어 특정 리소스에 접근을 요청할 때 사용하는 방식
     ...
     1. 권한 요청    |       5. 액세스 토큰으로 데이터에 접근      | 인증 서버에
        ...
        3. 인증 코드 제공
            사용자가 로그인에 성공하면 권한 요청 시에 파라미터로 보낸 redirect_url로 리다이렉션됩니다.
        ...
        4. 액세스 코드 응답
            '인증 코드를 받으면' 액세스 토큰으로 교환해야 합니다.
            엑세스 토큰은 로그인 세션에 대한 보안 자격을 증명하는 식별 코드를 의미.
