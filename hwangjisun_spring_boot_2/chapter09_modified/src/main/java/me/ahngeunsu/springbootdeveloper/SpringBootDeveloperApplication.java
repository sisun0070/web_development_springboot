        ...
        토큰 기반 인증 :
            * 토큰 : 서버에서 클라이언트를 구분하기 위한 유일한 값
            ...
            토큰을 전달하고 인증 받는 과정
                ...
                1) 클라이언트가 아이다와 비밀번호를 서버에게 전달하면서 인증 요청 ->
            ...
            토큰 기반 인증의 특징
                1) 무상태성
                    사용자의 인증 정보가 담겨 있는 토큰이 서버가 아닌 클라리언트에 있으므로
                        서버에 저장할 필요 X
                ...
                3) 무결성
                    토큰 기반 인증 방식은 HMAC(Hash-based Message Authenticatiopn) 기법이라고도 하는데,
                    토큰을 발급한 이후에는 토큰 정보를 변경하는 행위를 할 수 없음.
                    만약 토큰을 한 글자라도 변경하면 서버에서는 유효하지 않은 토큰이라고 판단합니다.
            JWT(Json Web Token) -> json 형태로 주고받는것임
                ...
                JWT 구조 : 온점(.)을 기준으로 헤더(header), 내용(payload), 서명(signature)로 이루어짐.
                    ...
                    2) 내용에는 토큰과 관련된 정보를 담습니다. 내용의 한 덩어리를 클레임(claim)
                        ...
                        비공개 클레임(private claim) : 공개되면 안되는 클레임
                          ...
                          - JWT 예시
                        {
                            "iss" : "maybeags@gamil.com" // 등록된 클레임
                            "iat" : 1622370678,
                            "exp" : 1622372678,
                            "http://ahngeunsu.com/jwt_claims/is_admain": ture, // 공개 클레임
                            "email": "maybeags@gamil.com",      // 비공개 클레임
                            "hello": "안녕하세요"
                        }

                        iss, iat, exp 는 JWT 자체에서 등록된 클레임
                        URI로 네이밍된 http://ahngeunsu.com/jwt_claim/is_admin : 공개 클레임
                        ...
                        3) 서명 : 해당 토큰이 조작되었거나 변경되지 않았음을 확인하는 용도로 사용.
                            ...
                            토큰 유효 기간
                                ...
                                토크 유효 기간이 너무 짧다면 사용자가 불편함.
                                    -> 리프레시 토큰의 개념이 발생
                                    * 리프레시 토큰 : 엑세스 토큰과 별개로 사용자를 인증하기 위한 용도가 아니라
                                        액세스 토큰이 만료되었을 때 엑세스 토큰을 발급하기 위해 사용.

        2. JWT 서비스 구현
            의존성과 토큰 제공자를 추가하고 리프레시 토큰 도메인 토큰 필터를 구현하고
            -> JWT 서비스를 사용할 준비가 됏다고 볼 수 있습니다.
