...
/*
    처음으로 SpringBootDeveloperApplication 파일을 실행시키면 whilteLabel error page가 뜸
    현재 요청에 해당하는 페이지가 존재하지 않기 때문에 생겨난 문제임
    -> 하지만 스프링 애플리케이션은 실행됨

    그래서 error 페이지가 기분 나쁘니까 기본적으로 실행될 때의 default 페이지 하나를 생성하겠음

    20241223 MON.
        1. IntelliJ Community Version 설치 -> bin.PATH 체크하고 나머지는 전부 디폴트
        2. Git 설치
        3. GitHub 연동 -> web_development_springboot -> 현재 문제가 좀 있음
        4. IntelliJ Gradle 및 SpringBoot 프로젝트를 생성
        5. POSTMAN을 설치
 */
