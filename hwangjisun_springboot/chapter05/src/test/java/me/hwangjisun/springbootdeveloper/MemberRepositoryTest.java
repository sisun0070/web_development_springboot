...
/*
    ...
    작성 후(혹은 확인 후에) 코드 의미 :
        scr/main/resources 폴더 내에 있는 data.sql 파일을 자동 실행 못하게 하는 코드
...
class MemberRepositoryTest {
    ...
    /*
        ...
        FROM member,           *(aterist : all), WHERE id = 2,    SELECT
     */
    @Sql("/insert-members.sql")
    @Test
    void getMemberById() {
    ...
    /*
        ...
        일반적인 경우 JPA가 정해준 메서드 이름 규칙에 따르면 쿼리문을 특별히 구현하지 않아도
        ...
        전체 조회 -> findAll()
        아이디로 조회 -> findById()
        특정 컬럼으로 조회(name) -> 쿼리 메서드 명명 규칙에 맞게 정의 후 사용 (find / get 차이)
    ...
    @Test       // @Sql애너테이션을 달지 않습니다. 이 메서드에서 1,'A'에 해당하는 객체를 저장할거라고
    void saveMember() {
        // given
