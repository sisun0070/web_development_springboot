...
-- RDBS에서는 데이터의 중복을 피하고 쉽게ㅐ 관리하기 위해 데이터를 여러 곳에 나누어 보관함.
-- 	ex) 어떤 주문을 담당한 직원을 확인하고 싶은데, 주문 아이디 orders 테이블에 있고,
-- 		직원 이름은 staff 테이블에 있으면 어떻게 쿼리를 입력해야 하는지 고민해보도록 합니다.
-- 		
-- 	이상에서 이루어지는 데이터 분산을 '데이터 정규화(date normalization)'라고 하며
...
-- user와 orders를 하나로 결합하여 출력합니다.(단, 줌누 정보가 있는 회원의 정보만 출력)
select *
...
-- orders에선 PK는 아니지만 JOIN을 수행할 때 users와 합치는 조건이 됩니다. 