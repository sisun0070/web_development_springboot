...
select * from users;
-- users에서 country가 Korea인 회원 중에서 미케팅 수신에 동의한 회원 수를 구해 출력할 것
...
-- users에서 country별로 마케팅 수신 동의한 회원 수와 동의하지 않은 회원 수를 구해 출력할 것
...
-- 국가별로는 오름차순(defulat라서 미표기), uniqueUserCnt를 기준으로는 내림차순
...
그래서 group BY에 뎌러 기준을 설정하면, 컬럼 순서에 따라 결과가 달라짐.
...
-- 		분할 가능한데, 컬럼 순서에 따라 결과에 영황을 미치므로 우선순위를 생각할 필요 있음
...
-- SUBSTR(컬럼명, 시작값, 글자개수)
-- users에서 월별(e.g. 2023-10) 가입 회원 수를 출력할 것.
...
-- 2. orders에서 staff_if 별, user_id 별로 주문 건수(COUNT(*))를 출력할 것.
...
-- WHERE절을 이용하는 방법이 있기 하지만 추가적인 개념에 대해서 학습할 예정입니다.
-- 언제나 WHERE을 쓰는 것이 용이하지 않다는 점부터 짚고 넘어가서 HAVING 학습을 할 예정
...
만약에 회원 수가 8명 이상인 국가의 회원 수만 필터링하려먼 어떡할까?
...
1. where에서 필터링을 시도할 때 오류가 발생하는 이유 :
	...
	하지만 집계 함수로 계산된 값의 경우엔, 그룹화 이후에 이루어지기 때문에
	순서상으로 group by 보다 뒤에 있어야 해서 where 절 도입이 불가능 함.
	
2. 즉 group by 를 사용한 집계 값을 필터링할 때는 -> HAVING을 사용

-- orders에서 담당 직원별 주문 건수와 주문 회원 수를 계산하고, 줌누 건수가 10건 이상이면서
-- 주문 회원 수가 40명 이하인 데이터만 출력(단, 주문 건수 기준으로 내림차순 정렬할 것. 
-- staff_id ,users_id, id 컬럼 이용)

select staff_id, COUNT(distinct id), COUNT(distinct id)
	from orders
	group by staff_id 
	having COUNT(distinct id) >= 10 and COUNT(distinct id) <= 40
	order by COUNT(distinct id) desc;

-- HAVING 정리
-- 순서상 GROUP BY 뒤쪽에 위치하며, GROUP BY와 집계함수로 그룹별 집계한 값을
-- 필터링할 때 사용
...
select country, count(distinct city), count(distinct id)
	...
	order by count(distinct id), count(distinct city) desc;
