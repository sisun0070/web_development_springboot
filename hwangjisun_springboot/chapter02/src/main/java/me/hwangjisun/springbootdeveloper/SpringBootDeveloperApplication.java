...
/*
    New project 생성 시 주의점 :
        ...
        6. 앞으로의 수업 시간에는 new project 생성시에 이렇게 자세히 풀이할 일은 자주 있지 않을 예정이니 꼭
            익혀두길 바람 -> github 본다고 되는 부분이 아니니만큼 신경써주시면 감사하겠습니다.
 */
...
/*
    1. Spring vs. SpringBoot
        ...
        SpringBoot의 등장
            ...
            즉, 스프링부트라는것은 기본적으로 스프링에 속해있는 도구
    ...
    2. 스프링 컨셉
        1) 제어의 역전(IoC)
            Inversion of Control : 다른 객체를 직접 생성하거나 제어하는 것이 아니라
                ...
                ex) 클래스 A에서 클래서 B의 객체를 생성한다고 가정했을 때 자바 코드
                public class A {
                    B b = new B();
                }
        ...
        2) 외존성 주입(DI)
            Dependency Injection : 어떤 클래스가 다른 클래서에 의존한다는 의미
        ...
        3) 빈과 스프링 컨테이너
            3)-1. 스프링 컨테이너 : 빈이 생성되고 소면되기까지의 생명주기를 관리함
            ...
            3)-2. 빈 : 스프링 컨테이너가 생성하고 관리하는 객체 -> 이상의 코드들에서 B가 빈에 해당
                ...
                public class NyBean{}
                    ...
                    소문자로 바꾸어서 관리함. 즉 클래스 MyBean의 빈 이름을 myBean이 됨
    ...
    3. 스프링 부트 3 두러보기
        ...
        스프링 부트 스타터 살펴보기
            ...
            spring-boot-starter-actuator : 모니터링을 위해 애플리케이션에서 제공하는 다양한 정보를
                제공하기 쉽게 하는 의존성 모음
            spring-boot-starter-jpa : ORM(추후 수업 예정)을 사용하기 위한 인터페이스의 모임인
                JPA를 더 쉽게 사용하기 위한 의존성 모음

        스프링 부트 3와 자바 버전
            스프링 부트 3는 자바 17버전 이후부터 사용 가능합니다 !!
